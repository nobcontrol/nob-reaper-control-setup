-- Reaper Script: Select items just below the current selection's bounding box
...
for i = 0, selectedItemCount - 1 do
    ...
    if not bottommostTrack or itemTrackNumber > reaper.GetMediaTrackInfo_Value(bottommostTrack, "IP_TRACKNUMBER") then
        bottommostTrack = itemTrack
    end
...
if not bottommostTrack or not earliestPos or not latestPos then return end
...
for i = 0, itemCount - 1 do
    ...
    local itemTrack = reaper.GetMediaItemTrack(item)
    local itemTrackNumber = reaper.GetMediaTrackInfo_Value(itemTrack, "IP_TRACKNUMBER")
    local bottommostTrackNumber = reaper.GetMediaTrackInfo_Value(bottommostTrack, "IP_TRACKNUMBER")

    -- Check if the item is above the bounding box
    if itemTrackNumber > bottommostTrackNumber and
    ((itemStart >= earliestPos and itemStart <= latestPos and itemEnd >= earliestPos and itemEnd <= latestPos) or
     ...
     (itemStart < earliestPos and itemEnd >latestPos))
     then
        local distance = itemTrackNumber - bottommostTrackNumber
