-- Lua script for REAPER to select next item to the right based on bounding box of selected items
  ...
  for track_num = 0, reaper.CountTracks(0) - 1 do
      if track_set[track_num] then
          ...
          for i = 0, reaper.CountTrackMediaItems(track) - 1 do
              ...
              local item_start = reaper.GetMediaItemInfo_Value(item, "D_POSITION")
              if (item_start > max_start and item_start < bestItemPosition) then
