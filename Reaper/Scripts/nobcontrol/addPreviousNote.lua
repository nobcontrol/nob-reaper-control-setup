...
function main()
  ...
  if #selectedNotes > 0 then
      -- Search for the note immediately to the left with the same pitch
      for i = 0, noteCount - 1 do
          local _, _, _, startppqpos, _, _, pitch, _ = reaper.MIDI_GetNote(take, i)
          if startppqpos < firstSelectedPPQ then
              for _, selNote in ipairs(selectedNotes) do
                  if pitch == selNote.pitch and (firstSelectedPPQ - startppqpos) < minTimeDiff then
                      minTimeDiff = firstSelectedPPQ - startppqpos
                      targetNoteIndex = i
                  end
              end
          end
          ...
          for i = 0, noteCount - 1 do
              ...
              if startppqpos < firstSelectedPPQ then
                  for _, selNote in ipairs(selectedNotes) do
                      ...
                      if (firstSelectedPPQ - startppqpos) < minTimeDiff or ((firstSelectedPPQ - startppqpos) == minTimeDiff and pitchDiff < minPitchDiff) then
                          minTimeDiff = firstSelectedPPQ - startppqpos
